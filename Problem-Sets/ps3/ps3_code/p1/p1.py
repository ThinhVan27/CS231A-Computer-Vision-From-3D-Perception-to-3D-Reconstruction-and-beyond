...
def form_initial_voxels(xlim, ylim, zlim, num_voxels):
    ...
    object_volume = diff(xlim) * diff(ylim) * diff(zlim)
    voxel_volume = object_volume / num_voxels
    voxel_size = voxel_volume ** (1/3)
    
    x = np.arange(xlim[0], xlim[1], voxel_size, dtype=np.float64)
    y = np.arange(ylim[0], ylim[1], voxel_size, dtype=np.float64)
    z = np.arange(zlim[0], zlim[1], voxel_size, dtype=np.float64)
...
def carve(voxels, camera):
    # TODO: Implement this method!
    sil_matrix = camera.silhouette
    image = camera.image
    M = camera.P
    
    N = voxels.shape[0]
    
    P = np.hstack((voxels, np.ones(shape=(N,1))))
    p = (M @ P.T).T
    
    p = p / p[:, -1].reshape(N, 1)
    
    mask = np.zeros(shape=(N,), dtype=np.int8)
    for i in range(N):
        x, y = np.int32(p[i][0]), np.int32(p[i][1])
        if x in range(sil_matrix.shape[1]) and y in range(sil_matrix.shape[0]):
            if sil_matrix[y,x] == 1.0:
                mask[i] = 1
    
    if np.count_nonzero(mask) > 2:
        voxels = voxels[mask==1, :]
    return voxels
