...
class ImageClassifyModel(object):
    ...
    def __init__(self, image_embed_net,
        ...
        '''
        if exclude_embed_params is True:
          self.parameters = self.image_classify_net.paramerters()
        else:
          self.parameters = nn.Sequential(self.image_embed_net, self.image_classify_net).parameters()
    def classify(self, image):
      # TODO revise to return output of image_classify_net
      image = self.image_embed_net(image)
      image = self.image_classify_net(image)
      return image
